...
class StringMethods
{
  ...
  private static array $_singular = array(
                                      "(matr)ices$" => "\\1ix",
                                      "(vert|ind)ices$" => "\\1ex",
                                      "^(ox)en" => "\\1",
                                      "(alias)es$" => "\\1",
  ...
  private static array $_plural = array(
                                        "^(ox)$" => "\\1\\2en",
                                        "([m|l])ouse$" => "\\1ice",
  ...
  public static function sanitize(string $string, ?array $mask): ?string
  {
    if (is_array($mask))
    {
      $parts = $mask;
  ...
  public function indexOf(string $string, string $substring,int $offset = null): int
  {
    $position = strpos($string, $substring, $offset);

    return !is_int($position) ? -1 : $position;

  }

  public function lastIndexOf(string $string,string $substring,int $offset = null): int
  {
